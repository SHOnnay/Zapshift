...
const AssignRiders = () => {
    ...
    const { data: parcels = [], refetch: parcelRefetch } = useQuery({
        ...
        queryFn: async () => {
            //fetch pending pickup parcels from the server
            const res = await axiosSecure.get('/parcels?deliveryStatus=pending-pickup');
            return res.data;
        }
    });

    //todo invalidate qeury 
    ...
    const openAssignRiderModal = (parcel) => {
        // Set the selected parcel and open the modal
                    ...
                    <tbody>
                        {
                            parcels.map((parcel, index) =>
                                <tr key={parcel._id} onClick={() => openAssignRiderModal(parcel)} className="cursor-pointer">
                                    ...
                                    <td>
                                        <button className="btn btn-primary text-secondary" onClick={() => openAssignRiderModal(parcel)}>
                                            Find Riders
                                        </button>
                                    </td>
                                </tr>
                            ...
                            <tbody>
                                {
                                    riders.map((rider, index) =>
                                        <tr key={rider._id}>
                                            ...
                                            <td>
                                                <button
                                                    onClick={() => handleAssignRider(rider)} className="btn btn-primary text-secondary">Assign</button>
                                            </td>
                                        </tr>
                                    )
                                }
